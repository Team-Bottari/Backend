...
class LikeSource:
    ...
    async def create_like(self, posting_member_info:Like_create, member_by_token: Member = Depends(get_current_member_by_token)):
        ...
        try:
            ...
            if posting['_source']['remove'] == True:
                return {"response":"해당 게시물이 삭제되었습니다."}
            else: 
                # 이미 누른 좋아요 인지 check(사실상 클릭을 막아야함.)
                query = select(Like).where(Like.posting_id == posting['_id'], Like.member_id == member_from_token['member_id'], Like.status == True)
                ...
                if like is not None:
                    ...
                    like = Like(posting_id = posting_member_info['posting_id'], member_id =member_from_token['member_id'], status=True)
                    session.add(like)
    ...
    async def cancle_like(self, posting_member_info: Cancle_like_create, member_by_token: Member = Depends(get_current_member_by_token)):
        posting_member_info = jsonable_encoder(posting_member_info)
        member_from_token = jsonable_encoder(member_by_token.first()[0])
        # posting 유효한 포스팅인지 확인
        try:
            ...
            query = select(Like).where(Like.posting_id == posting_member_info['posting_id'], Like.member_id == member_from_token['member_id'], Like.status == True)
            result = await session.execute(query)
            like = result.first()
            if like is None:
                ...
                query = update(Like).where(Like.posting_id == posting_member_info['posting_id'], Like.member_id == member_from_token['member_id'], Like.status == True).values(status = False)
    ...
    async def like_list(self, member_by_token: Member = Depends(get_current_member_by_token)):
        member_from_token = jsonable_encoder(member_by_token.first()[0])
        
        query = select(Like).where(Like.status == True, Like.member_id == member_from_token['member_id'])
        result = await session.execute(query)
